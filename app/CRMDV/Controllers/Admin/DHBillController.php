...
//            ['name' => 'rate_content', 'type' => 'custom', 'td' => 'CRMDV.dhbill.list.td.rate_content', 'label' => 'Đánh giá', 'sort' => true],
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('CRMDV.dhbill.list')->with($data);
    }
    ...
    public function getDataList(Request $request) {
        ...
        //  Tính điểm
        //  id người tính điểm
        $nv_id = $request->has('nv_id') ? $request->nv_id : \Auth::guard('admin')->user()->id;
...
//        dd((float) @$diem_dh[14]);
    ...
    public function appendWhere($query, $request)
    {
        ...
        if (@$request->nv_id != null) {
            $bill_ids = BillProgress::where(function ($query) use ($request) {
                            $query->orWhere('dh_id', $request->nv_id);    //  tìm các dự án điều hành
                            $query->orWhere('kt_id', $request->nv_id);    //  tìm các dự án kỹ thuật
                        })
                        ->pluck('bill_id')->toArray();
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMDV.dhbill.edit')->with($data);
            }
            ...
            if ($item->save()) {
                ...
                if ($request->log_name != null || $request->log_note != null) {
                    ...
                    $leadController = new \App\CRMDV\Admin\LeadController();
