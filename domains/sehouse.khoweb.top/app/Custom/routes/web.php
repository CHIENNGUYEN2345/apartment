...
Route::get('diem-danh', function(\Illuminate\Http\Request $r) {
    ...
    $timekeeper = Modules\WebBill\Models\Timekeeper::where('may_cham_cong_id', $admin->may_cham_cong_id)->where('time', '>=', date('Y-m-d 00:00:00'))->first();

    if (!is_object($timekeeper)) {
        $timekeeper = new Modules\WebBill\Models\Timekeeper();
